...
function buildMarkdown(run) {
  ...
  const scored = rs.filter((r) => !r.error && r.delta != null);
  const avg = (f) => (scored.length ? scored.reduce((a, r) => a + f(r), 0) / scored.length : null);
  const cls = (d) => (d >= 0.03 ? 'improvement' : d <= -0.03 ? 'deficiency' : 'neutral');
  const L = [];
  L.push(`# NeuraNet Batch Report`);
  L.push('');
  L.push(`- Date: ${new Date(run.date).toLocaleString()}`);
  L.push(`- Questions: ${rs.length} (scored: ${scored.length})`);
  if (scored.length) {
    L.push(`- Avg quality guided: **${avg((r) => r.quality).toFixed(3)}** vs baseline **${avg((r) => r.baselineQuality).toFixed(3)}**`);
    L.push(`- Average infrastructure delta: **${avg((r) => r.delta) >= 0 ? '+' : ''}${avg((r) => r.delta).toFixed(3)}**`);
  }
  L.push('');
  L.push('| # | Question | Mode | Sim | Guided | Baseline | Delta | Verdict |');
  L.push('|---|----------|------|-----|--------|----------|-------|---------|');
  rs.forEach((r, i) => {
    const c = r.error ? 'failed' : cls(r.delta ?? 0);
    L.push(`| Q${i + 1} | ${(r.task || '').replace(/\|/g, '/')} | ${r.variant === 'transfer' ? `transfer (${r.sim?.toFixed?.(2)})` : r.variant} | ${r.variant === 'transfer' && r.sim ? r.sim.toFixed(2) : '-'} | ${r.quality?.toFixed(2) ?? '-'} | ${r.baselineQuality?.toFixed(2) ?? '-'} | ${r.delta != null ? (r.delta >= 0 ? '+' : '') + r.delta.toFixed(2) : '-'} | ${c} |`);
  });
  L.push('');
  L.push('_Generated by NeuraNet Console - quality is a heuristic form/relevance proxy, not factual ground truth._');
  return L.join('\n');
}
...
export default function BatchAnalysis() {
  ...
  /* every non-empty line counts; tiny lines are flagged, not silently dropped */
  const parsed = useMemo(() => {
    const lines = raw.split('\n').map((s) => s.trim()).filter(Boolean);
    return {
      questions: lines.filter((s) => s.length >= MIN_LEN),
      ignoredShort: lines.filter((s) => s.length > 0 && s.length < MIN_LEN).length,
    };
  }, [raw]);
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08, duration: 0.35 }} className="mt-8" ref={composerRef}>
        <Glass>
          <div className="p-6">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className={`mono-num rounded-full px-3 py-1 text-[11.5px] ${
                  ...
                  {Math.min(questions.length, MAX_Q)} / {MAX_Q} question{questions.length > 1 ? 's' : ''}
                </span>
              </div>
            </div>
            ...
            {parsed.ignoredShort > 0 && (
              <p className="mt-2 text-[11.5px] text-low">
                {parsed.ignoredShort} line{parsed.ignoredShort > 1 ? 's' : ''} under {MIN_LEN} characters ignored.
              </p>
            )}
          </div>
        </Glass>
      ...
      {!running && progress.total > 0 && results.length === progress.total && !isHistorical && (
        ...
        >
          ...
          <div className="flex items-center gap-2.5">
            <button
              onClick={() => {
                const run = { id: Date.now(), date: new Date().toISOString(), questions: questions.slice(0, MAX_Q), results };
                download(`neuranet-batch-${new Date().toISOString().slice(0, 16).replace(/[:T]/g, '-')}.json`, JSON.stringify(run, null, 2), 'application/json');
              }}
              className="rounded-lg border border-line bg-ink-850 px-4 py-2 text-[12.5px] font-medium text-mid hover:text-hi"
            >
              Export JSON
            </button>
            <button
              onClick={() => {
                const run = { id: Date.now(), date: new Date().toISOString(), questions: questions.slice(0, MAX_Q), results };
                download(`neuranet-batch-report.md`, buildMarkdown(run), 'text/markdown');
              }}
              className="rounded-lg border border-line bg-ink-850 px-4 py-2 text-[12.5px] font-medium text-mid hover:text-hi"
            >
              Export Markdown
