  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO experiences (organization_id, agent_id, task_id, outcome, strategy, search_queries, sources, domain, quality_score, confidence_score, trust_score, verification_status, visibility, provenance, freshness_score)
       VALUES ($1, $2, $3, $4, $5::jsonb, $6::jsonb, $7::jsonb, $8, $9, $10, $11, $12, $13, $14::jsonb, $15)
      ...
      [orgId, agentId, task_id, outcome,
       strategy ? JSON.stringify(strategy) : null,
       search_queries ? JSON.stringify(search_queries) : null,
...
router.post('/recommend', authenticateApiKey, async (req, res) => {
  try {
    const { task_id, original_task, domain: bodyDomain } = req.body;
    const orgId = req.organization_id;
    ...
    if (task_id) {
      ...
      task = tasks[0];
    } else if (original_task) {
      // Allow recommendations without persisted task - use provided text directly
      task = {
        id: task_id || null,
        original_task: original_task,
        domain: bodyDomain || 'general',
        normalized_task: null,
        agent_id: null
      };
    } else {
      return res.status(400).json({
        error: 'Task ID or original_task is required for recommendations',
        request_id: req.request_id
      });
    }
    ...
    const { rows: experiences } = await pool.query(`
      SELECT e.id, e.organization_id, e.agent_id, e.task_id, e.outcome, e.strategy, e.domain,
             ...
             e.provenance, e.created_at, a.name AS agent_name,
              ...
              CASE WHEN e.domain = $2 THEN 1.0 ELSE 0.0 END AS domain_match,
       ...
       ORDER BY
         ...
         (e.trust_score * 0.3 +
          ...
          CASE WHEN e.domain = $2 THEN 1.0 ELSE 0.0 END * 0.25 +
          CASE WHEN e.reuse_count > 0 THEN 0.15 ELSE 0 END +
          CASE WHEN e.failure_count = 0 THEN 0.05 ELSE 0 END) DESC
       LIMIT 10
    `, [orgId, task.domain || 'finance']);
...
router.post('/strategies/recommend', authenticateApiKey, async (req, res) => {
  try {
    ...
    if (task_id) {
      ...
      return res.status(400).json({
        error: 'Task ID or original_task is required for strategy recommendation',
        request_id: req.request_id
      });
    }
    ...
    const { rows: experiences } = await pool.query(`
      SELECT e.id, e.organization_id, e.agent_id, e.task_id, e.outcome, e.strategy, e.domain,
       ...
       WHERE e.organization_id = $1
         ...
         AND e.trust_score > 0
       ORDER BY
        (e.trust_score * 0.3 + COALESCE(e.freshness_score, 0.5) * 0.2 + CASE WHEN e.domain = $2 THEN 1.0 ELSE 0.0 END * 0.25 + CASE WHEN e.reuse_count > 0 THEN 0.15 ELSE 0 END + CASE WHEN e.failure_count = 0 THEN 0.05 ELSE 0 END) DESC
      LIMIT 10
    `, [orgId, task.domain || 'finance']);
